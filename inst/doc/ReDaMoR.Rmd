...
The aim of [TKCat](https://github.com/patzaw/TKCat) (Tailored Knowledge Catalog)
...
The example below shows how to build a `file_MDB` object for the the **subset**
of the [HPO](https://hpo.jax.org/app/)<sup>([1](#hpoRef))</sup> which
is provided within the package.

```{r, eval="TKCat" %in% rownames(installed.packages())}
library(TKCat)
...
Here are some basic operations that can be applied on these `MDB` objects.
...
`MDB` objects can also be subsetted, combined, filtered, joined and merged.
