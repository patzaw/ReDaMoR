  ...
  if (length(x$indexes) > 0) {
    for (i in 1:length(x$indexes)) {
      if (x$indexes[[i]]$unique && length(x$indexes[[i]]$fields) == 1) {
        if (
          x$fields[
            which(x$fields$name == x$indexes[[i]]$fields),
            "type"
          ] %in%
            c("row", "column")
        ) {
...
confront_table_data <- function(
  ...
  if (is.MatrixModel(x)) {
    ...
    if (!inherits(d[1], vf$type)) {
      ...
      toRet$success <- FALSE
      toRet$fields[[vf$name]]$message <- paste(
        c(
...
identical_RelTableModel <- function(x, y, includeDisplay = TRUE) {
  ...
  ## Primary key ----
  toRet <- toRet &&
    length(x$primaryKey) == length(y$primaryKey) &&
  ...
  if (includeDisplay) {
    toRet <- toRet &&
      length(x$display) == length(y$display) &&
      (length(x$display) == 0 ||
        identical(
