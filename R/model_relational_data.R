  ...
  shiny::shinyUI(shiny::fluidPage(
    ...
    shiny::tags$head(
      ...
      shiny::tags$link(
        rel = "stylesheet",
        type = "text/css",
        href = "www/cerulean.css"
      ),
      shiny::tags$link(
        rel = "stylesheet",
        type = "text/css",
        href = "www/defChanges.css"
      ),
      shiny::tags$link(
        rel = "stylesheet",
        type = "text/css",
        href = "www/appElements.css"
      ),
...
buildServer <- function(
  ...
  function(input, output, session) {
    ...
    model <- shiny::reactiveValues(
      ...
      indexTable = dplyr::tibble(
        # Indexes of the table to edit
        fields = character(),
        unique = logical()
      ),
      fieldTable = dplyr::tibble(
        # Fields of the table to edit
        name = character(),
        type = character(),
        nullable = logical(),
        unique = logical(),
    ...
    output$modelSummary <- shiny::renderUI({
      ...
      shiny::tagList(
        ...
        nt,
        "-",
        shiny::tags$strong("Foreign keys:"),
        nfk,
        "-",
        shiny::tags$strong("Fields:"),
        np
      )
    })
    ...
    modelNet_selectedEdges <- shiny::reactive({
    ...
    shiny::observe({
      ...
      if (
        !is.null(cmn$nodes) > 0 &&
          nrow(cmn$nodes) > 0 &&
          all(!is.na(cmn$nodes$x)) &&
          all(!is.na(cmn$nodes$y))
      ) {
        toReplot <- FALSE
    ...
    shiny::observe({
      takeAction <- ((!is.null(input$removeTables) && input$removeTables > 0))
    ...
    shiny::observe({
      takeAction <- ((!is.null(input$duplicateTables) &&
        input$duplicateTables > 0))
      shiny::req(takeAction)
      tns <- shiny::isolate(selection$tables)
      m <- shiny::isolate(model$x)
    ...
    shiny::observe({
      if (
        length(foreignKey$fromTable) == 0 ||
          length(foreignKey$toTable) == 0 ||
          length(foreignKey$fromFields) == 0 ||
          length(foreignKey$toFields) == 0
      ) {
    ...
    output$fkFields <- shiny::renderUI({
      ...
      shiny::fluidRow(
        ...
        shiny::column(1, shiny::uiOutput("rmFkField"), class = "rightBox")
      )
    })
    ...
    output$ilcard <- shiny::renderUI({
      ...
      m <- shiny::isolate(model$x)
      cmin <- if (tns[1] == ft) {
        shiny::isolate(foreignKey$fmin)
      } else {
        shiny::isolate(foreignKey$tmin)
      }
      cmax <- if (tns[1] == ft) {
        shiny::isolate(foreignKey$fmax)
      } else {
        shiny::isolate(foreignKey$tmax)
      }
    ...
    shiny::observe({
      takeAction <- ((!is.null(input$removeFK) && input$removeFK > 0))
      shiny::req(takeAction)
    ...
    shiny::observe({
      ...
      selection$tables <- sort(intersect(
        shiny::isolate(selection$tables),
        names(m)
      ))
      selection$fk <- sort(intersect(
        shiny::isolate(selection$fk),
        mn$edges$id
      ))
    })
